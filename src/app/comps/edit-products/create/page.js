...
export default function Page({ params }) {
    ...
    const handleSubmit = async () =>{
        ...
        const req = await fetch(`http://localhost:3000/api/product/`, {method: 'POST', body: JSON.stringify(productInfo)})
        if (req.status === 200) {
        ...
        <main className='min-h-screen bg-dark-beige'>
            <div className="flex justify-center items-center h-screen">
                <div className="bg-beige p-8 rounded-lg shadow-lg w-full max-w-md">
                    <h2 className="text-2xl font-bold mb-4 text-gg">Dodaj produkt</h2>
